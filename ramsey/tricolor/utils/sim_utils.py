...
def end_game_eval(game):
    ...
    if last_player == Player.p1:
        ...
        last_player = 'player2'
    result = 'D'
    colored_edges = game.colored_edge_list
    num_edges = int(len(colored_edges)/2)
    num_red = len(game.red_subgraph.edges)
    num_blue = len(game.blue_subgraph.edges)
    num_green = len(game.green_subgraph.edges)
    num_moves = num_red + num_blue + num_green
    winning_color = None
    winning_clique = ()
    ...
    green_cliques = game.green_cliques
    winning_green_clique_order = game.green_clique_order
    try:
    ...
    if len(largest_green_clique) >= winning_green_clique_order:
            winning_color = 'green'
            winning_clique = largest_green_clique
            result = 'W'
...
def game_info(game):
    ...
    num_green = len(list(game.green_subgraph.edges))
