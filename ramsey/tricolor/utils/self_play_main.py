...
from tricolor.encoders.experience import TricolorExperienceCollector
from tricolor.encoders.experience import combine_experience, save_experience
...
def game_sim(board, clique_orders, player1, collector1, player2, collector2, head_start = None):
    ...
    if result == 'D':
        ...
        # the last player to make a play was the winner of the game.
...
def parallel_self_play_games(trained_model, encoder_name, host_graph_order,
                             graph_order, clique_orders,
                             MCTS_rounds, MCTS_temp,
                             num_batches, games_per_batch, exp_dir, worker_ID,
                             head_start = None):
    model = load_model(f'tricolor/models/{trained_model}')
    encoder = TricolorEncoder.get_encoder_by_name(encoder_name, host_graph_order)
    board = Board(nx.complete_graph(graph_order))
    player1 = TricolorAgent(model, encoder, MCTS_rounds, MCTS_temp)
    player2 = TricolorAgent(model, encoder, MCTS_rounds, MCTS_temp)
    num_wins = 0
    total_moves = 0
    long_games = 0
    num_games = num_batches * games_per_batch
    for i in range(num_batches):
        collector1 = TricolorExperienceCollector()
        collector2 = TricolorExperienceCollector()
        exp_tag = f'worker{worker_ID}_batch{i+1}'
        for i in range(games_per_batch):  
            player1.set_collector(collector1)
            player2.set_collector(collector2)
            sim = game_sim(board, clique_orders, player1, collector1, player2, collector2, head_start)
            result = sim[0]
            num_moves = sim[1]
            if num_moves == 10:
                long_games +=1 
...
def combine_files(exp_dir):
    directory = f'tricolor/experience/{exp_dir}'
