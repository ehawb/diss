...
from tricolor.agent.agent_utils import get_indices, get_edges, get_priors
from tricolor.agent.agent_utils import host_edge_list, ends_game
from tricolor.board import Move
from random import choice
from tricolor.encoders.experience import TricolorExperienceCollector, TricolorExperienceBuffer
...
class ZeroTreeNode:
    ...
    def is_node_terminal(self):
        ...
        green_order = self.state.green_clique_order
        max_red = 0
        max_blue = 0
        max_green = 0
        if self.last_move is None:
            ...
            try:
                ...
                max_blue = 0
            green_check = self.state.check_monochromatic_cliques(self.last_move, 'green')
            try:
                max_green = max(green_check, key = len)
                max_green = len(max_green)
            except ValueError:
                max_blue = 0
...
class TricolorAgent(Agent):
    def __init__(self, model, encoder, rounds_per_move = 250, c = .5):
        self.model = model
        self.encoder = encoder
        self.host_graph_order = encoder.size
        self.collector = None
    ...
    def game_info(self, game):
        ...
        print(f'      green cliques: {game.green_cliques}')
        print(f'      green edges: {game.green_subgraph.edges}')
        print(f'      p1 edges: {game.p1_edges}')
        print(f'      p2 edges: {game.p2_edges}')
        print(f'      next player: {game.player}')
        
    def select_move(self, game_node):
        ...
        print(f'some info about the root:')
        root.get_info()
        input('[enter]')
        ...
        for move in root.moves():
            m_visits = root.visit_count(move)
            if m_visits != 0:
                moves_visited+=1
                print(f'{move} -- T --> {m_visits}')
                print(f'          R     {root.recent_visits(move)}')
        print(f'{moves_visited} different moves visited.')
        input('[enter]')
    ...
    def collect_info(self, game_state, root):    
        ...
        self.collector.record_decision(
            root_state_tensor, visit_counts, shifted_states, shifted_visits)    
    ...
    def create_node(self, game_state, move = None, parent = None):
        ...
        # (needed for when working on smaller graphs than what was used
        # in training)
        host_edges = host_edge_list(self.host_graph_order)
...
def shuffle_game_data(experience):
    ...
    shuff_exp = TricolorExperienceBuffer(states = shuff_states,
