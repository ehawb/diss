...
class ZeroTreeNode:
    ...
    def record_visit(self, move, value):
        print(f'      recording visit to {move} with value {value}.')
        self.total_visit_count += 1
        self.branches[move].visit_count += 1
        self.branches[move].total_value += value
        self.branches[move].recent_visit_count += 1
        print(f'        total visits: {self.total_visit_count}')
        print(f'        visits to move: {self.branches[move].visit_count}')
        print(f'        total value of move: {self.branches[move].total_value}')
        print(f'        recent visits to move: {self.branches[move].recent_visit_count}')
        # input('[enter]')
    
class InteractiveZeroAgent(Agent):
    ...
    def select_move(self, game_node):
        ...
        for i in range(self.num_rounds):
            ...
            print('walking down the tree...')
            # start at the root
            node = root
            # select the branch with the highest UCT score
            next_move = self.select_branch(node)
            print(f'    exploring the next move: {next_move}')
            is_over = False
            ...
            while node.has_child(next_move):
                ...
                try:
                    ...
                    next_move = next_move
                    break
            print(f'    out of the tree walk loop. terminal? {is_over}')
            ...
            print(f'    the value of the child node is {child_node.value}')
            move = next_move
            value = - 1 * child_node.value
            original_value = value
            print('time to walk up the tree.')
            steps = 1
            while node is not None:
                ...
                # if steps > 4 and original_value in [1, -1]:
                    # input('        applied a steep discount to move! [enter]')
        ...
        print(f'value of node being returned: {next_node.value}')
